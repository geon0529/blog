...
export default function Universe() {
  ...
  class Star {
    ...
    opacity: number = 0;
    opacityTresh: number = 0;
    do: number = 0;

    reset() {
      const { width, height, speedCoeff, first } = configRef.current;
      ...
      this.dx =
        ...
        (this.comet ? 1 : 0) * speedCoeff * getRandInterval(50, 120) +
        speedCoeff * 2;
      this.dy =
        -getRandInterval(speedCoeff, 6 * speedCoeff) -
        (this.comet ? 1 : 0) * speedCoeff * getRandInterval(50, 120);
      ...
      this.fadingIn = true;
      this.opacity = 0;
      this.opacityTresh = getRandInterval(0.2, 1 - (this.comet ? 1 : 0) * 0.4);
      this.do = getRandInterval(0.0005, 0.002) + (this.comet ? 1 : 0) * 0.001;
    }
    ...
    draw() {
      ...
      const { giantColor, cometColor, starColor } = configRef.current;
      universe.beginPath();
  ...
  const windowResizeHandler = () => {
    ...
    config.starCount = config.width * config.starDensity;
  ...
  const createUniverse = () => {
    ...
    universeRef.current = canvas.getContext("2d");
    const config = configRef.current;
  ...
  const draw = () => {
    ...
    universe.clearRect(0, 0, config.width, config.height);
  ...
  return (
    <div className="fixed top-0 left-0 -z-[10000] w-full h-full flex justify-center items-center bg-black p-0 m-0 contrast-[120%]">
      <div
      ...
      >
        <div className="w-full h-full">
          ...
          <div
            ...
            style={{ height: "min(315px, 30vh)" }} // 최대 315px, 화면의 30% 높이
          >
            <svg
              id="scene"
              className="absolute bottom-0 left-0 w-full"
              viewBox="0 0 1600 315"
              preserveAspectRatio="xMidYMax slice" // 하단 중앙 기준으로 맞춤
              style={{
                height: "315px", // 고정 높이
                minWidth: "100%", // 최소 너비 보장
                transform: "translateY(0)", // 위치 고정
              }}
            >
              <path
                id="ground"
